...
@property (nonatomic, copy) RCTDirectEventBlock onSaveImage;
...
- (void)longPressed:(UITapGestureRecognizer*)recognizer
{
  ...
  [self evaluateJavaScript:js completionHandler:^(NSString *imageUrl, NSError *error) {
    ...
    UIAlertAction *firstAction = [UIAlertAction actionWithTitle:@"保存"
    ...
    UIAlertAction *secondAction = [UIAlertAction actionWithTitle:@"取消"
...
- (void)image:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo
{
  if (_onSaveImage) {
    if (error) {
      _onSaveImage(@{@"state": @"fail", @"msg": error.localizedDescription});
    } else {
      _onSaveImage(@{@"state": @"success", @"msg": @"保存成功"});
    }
  }
}
